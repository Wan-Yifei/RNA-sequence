...
### 2.4 MA图
  MA-plot是一种展示样本间表达倍数差异的图像，因为数据都被取过对数所以被称为对数表达差异倍数（log2 fold change）。两样本间的对数表达差异倍数被称为M-value，而各个转录本表达平均值被称为A-value。其中M作为y值，A作为x值。以`control_1`与`treatment_1`为例，作图代码如下：
```{r}
M <- logdata$control_1 - logdata$treatment_1                        ## 计算对数表达差异倍数M-value
A <- (logdata$control_1 + logdata$treatment_1)/2                    ## 计算表达平均值
...
ggplot(MA, aes(x = A, y = M)) +                                     
  geom_point(aes(color=judge), size = 1.5, alpha = 1/5) +           ## 利用judge为数据点分类赋予颜色
  geom_hline(aes(yintercept=0), color = "blue3") +                  ## 添加水平线
  stat_smooth(se = FALSE, method = "loess", color = "red3")         ## 添加趋势线

```
  MA图中蓝色标明了上调的转录本，红色则是下调，在x轴附近的绿点是没有变化的转录本。当这只是一个粗略结果，还不能说具有统计差异。同时平均表达水平2.5以下的转录本呈射线状，非常整齐和对称，这暗示这些数据主要来自测序系统自身的噪音，应该过滤掉。所以我们后文将采用2.5作为阀值过滤低表达数据。
