...
```
  ...
  MA图中蓝色标明了上调的转录本，红色则是下调，在x轴附近的绿点是没有变化的转录本。这只是一个粗略结果，还不能说具有统计差异。同时平均表达水平5以下的转录本呈射线状，非常整齐和对称，这暗示这些数据主要来自测序系统自身的噪音，应该过滤掉。所以我们后文将采用5作为阀值过滤低表达数据。另外，呈箭头状的图像主体（称为Fan-shape)应以x轴为对称轴近似对称，因为基于假设大部分的转录本都没有显著变化。添加的红色趋势线显示了实际的轴线，与x轴有一定偏移。这再次证明数据归一化是需要的。
### 2.5 变化倍率散点图
